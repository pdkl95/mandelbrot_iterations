(function() {
  ...
  MandelIter = (function() {
    ...
    function MandelIter(context) {
      ...
      this.hide_highlight_msg = bind(this.hide_highlight_msg, this);
    ...
    MandelIter.prototype.init = function() {
      ...
      window.addEventListener('resize', this.on_content_wrapper_resize);
      ...
      this.keyhelp_dialog = new Dialog.Static('keyhelp', 'Keyboard Controls');
      ...
      this.load_from_file_uploader = new FileIO.Uploader('load_from_file_input', 'load_from_file');
      this.load_from_file_uploader.on_upload(this.on_load_from_file_upload);
    ...
    MandelIter.prototype.on_load_from_file_upload = function(filedata) {
      var objs, ref;
      objs = (ref = JSON.parse(filedata)) != null ? ref.saved_locations : void 0;
      if (objs != null) {
        return this.saved_locations.load_json_objs(objs);
      }
    };
    ...
    MandelIter.prototype.deferred_fit_canvas_to_width = function() {
      if (performance.now() < this.deferred_fit_time) {
    ...
    MandelIter.prototype.on_content_wrapper_resize = function(event) {
      if (this.defer_resize) {
        ...
        return this.deferred_fit_time = performance.now() + 200;
      } else {
        console.log('setting deferred fit timeout');
        this.defer_resise = true;
        this.deferred_fit_time = performance.now() + 500;
        return setTimeout(this.deferred_fit_canvas_to_width, 510);
      }
    };
    ...
    MandelIter.prototype.clear_julia = function() {
      if (this.ljopt.needs_clear) {
        this.ljopt.needs_clear = false;
        return this.graph_julia_ctx.clearRect(0, 0, this.graph_width, this.graph_height);
      }
    };

    MandelIter.prototype.draw_local_julia = function(lj) {
      var aax, aay, iter, j, k, l, o, pos1x, pos4x, px, py, q, ref, ref1, ref10, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, rowcount, s, start_time, val, x, xx, y, yy;
        ...
        if (this.ljopt.do_early_stop) {
          if ((y % 10) === 0) {
            if ((performance.now() - start_time) > this.julia_max_rendertime) {
              this.clear_julia();
              this.graph_julia_ctx.putImageData(this.current_image, this.local_julia.x, this.local_julia.y, 0, this.ljopt.ystart, this.local_julia.width, this.local_julia.height - this.ljopt.ystart);
    ...
    MandelIter.prototype.update_pause_anim = function() {
      ...
      if (this.pause_anim.finished()) {
        this.msg.style.color = this.pause_anim.finished_color();
